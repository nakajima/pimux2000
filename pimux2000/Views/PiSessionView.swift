...
struct PiSessionView: View {
	...
	@State var liveStreamState: LiveStreamState = .idle
	...
	@State var transcriptFreshness: PimuxTranscriptFreshness?
	// Latest active/attached status reported for the session.
	@State var transcriptActivity: PimuxSessionActivity?
	...
	@State var draftImages: [ComposerImage] = []
	// Optimistic local messages shown until the server transcript confirms them.
	@State var pendingMessages: [PendingLocalMessage] = []
	...
	@State var customCommands: [PimuxSessionCommand] = []
	...
	func beginAwaitingAgentActivity() {
	...
	func loadCustomCommands(force: Bool = false) async {
		guard let pimuxServerClient else { return }
